...
def predict_labels(out_shape, images, model_options, image_pyramid=None):
  """Predicts segmentation labels.
  ...
  for output in sorted(outputs_to_scales_to_logits):
    ...
    #predictions[output] = tf.argmax(scales_to_logits[_MERGED_LOGITS_SCOPE], 3)

  return predictions


def predict_logits(out_shape, images, model_options, image_pyramid=None):
  outputs_to_scales_to_logits = multi_scale_logits(
      images,
      model_options=model_options,
      image_pyramid=image_pyramid,
...
def multi_scale_logits_val(images,
                       model_options,
                       image_pyramid,
                       weight_decay=0.0001,
                       is_training=False,
