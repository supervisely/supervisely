...
network *load_network_sly(char *cfg, char *weights, int clear, int cutoff)
{
    ...
    if(weights && weights[0] != 0){
        if (cutoff <= 0)
            cutoff = net->n;
        load_weights_upto(net, weights, 0, cutoff);
    }
    // printf("\n\nnet->seen: %zu\n\n", *net->seen);
    if(clear) (*net->seen) = 0;
    return net;
...
float get_current_rate(network *net)
{
    ...
    int i;
    float rate;
    // printf("LR=%f\n", (float)(net->learning_rate));
    if (batch_num < net->burn_in) {
        float r = net->learning_rate; //* pow((float)batch_num / net->burn_in, net->power);
        //printf("bnum: %f \n",   (float)(batch_num));
        //printf("burnin: %f \n", (float)(net->burn_in));
        //printf("power: %f \n",  (float)(net->power));
        //printf("Result lr: %f \n", r);
...
detection *make_network_boxes(network *net, float thresh, int *num)
{
    ...
    // printf("YOLO num detections: %d with thresh %f\n", nboxes, thresh);
    if(num) *num = nboxes;
...
void forward_network_gpu(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
        net.index = i;
        layer l = net.layers[i];
        // printf("todo: layer %03d / %03d  %20s\n", i, net.n, get_layer_string(l.type));
        if(l.delta_gpu){
