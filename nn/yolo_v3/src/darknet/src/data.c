...
int *get_random_indices(int n, int m)
{
...
box_label *read_supervisely_boxes(float *gt_boxes, int num_gt_boxes)
{
    float x, y, h, w;
    int  i;
    box_label *boxes = calloc(num_gt_boxes + 1, sizeof(box_label));
    //printf("num %d\n", num_gt_boxes);
    for (i = 0; i < num_gt_boxes; ++i)
    {
        x = gt_boxes[i * 5 + 1];
        y = gt_boxes[i * 5 + 2];
        w = gt_boxes[i * 5 + 3];
        h = gt_boxes[i * 5 + 4];
        boxes[i].id = (int)gt_boxes[i * 5];
        boxes[i].x = x;
        boxes[i].y = y;
        boxes[i].h = h;
        boxes[i].w = w;
...
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, image sized)
{
    char labelpath[4096];
    find_replace(path, "images", "labels", labelpath);
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
    ...
    for (i = 0; i < count; ++i) {
        ...
        // b.w = w;
        // b.h = h;
        //draw_bbox(sized, b, 1, 1,0,0);
        //save_image(sized, "test_boxes");
        if ((w < .001 || h < .001)) {
            ++sub;
            continue;
        }
...
void fill_truth_detection_supervisely(int num_gt_boxes, float *gt_boxes, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, image sized)
{
    int count = num_gt_boxes;
    //printf("%f\n", gt_boxes[0]);
    box_label *boxes = read_supervisely_boxes(gt_boxes, num_gt_boxes);
    randomize_boxes(boxes, count);
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);
    if(count > num_boxes) count = num_boxes;
    float x,y,w,h;
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        float scale = rand_uniform(.75, 2);
        //float scale = 1;//rand_uniform(1.0, 2.0);
        float nw, nh;
...
data load_data_supervisely(int n, char **paths, float **boxes, int *num_gt_boxes, int m, int w, int h, int num_boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    //char **random_paths = get_random_paths(paths, n, m);
    int *random_indices = get_random_indices(n, m);
    int i, index;
    data d = {0};
    d.shallow = 0;
    d.X.rows = n;
    d.X.vals = calloc(d.X.rows, sizeof(float*));
    d.X.cols = h*w*3;
    d.y = make_matrix(n, 5*num_boxes);
    for(i = 0; i < n; ++i){
        index = random_indices[i];
        // printf("%s\n", paths[index]);
        image orig = load_image_color(paths[index], 0, 0);
        image sized = make_image(w, h, orig.c);
        fill_image(sized, .5);
        ...
        float new_ar = (orig.w + rand_uniform(-dw, dw)) / (orig.h + rand_uniform(-dh, dh));
        float scale = rand_uniform(.25, 2);  // original scale
        float nw, nh;
        ...
        // printf("ORIG w=%d h=%d\n", orig.w, orig.h);
        // printf("w=%d h=%d\n", w, h);
        // printf("nw=%f nh=%f dx=%f dy=%f\n", nw, nh, dx, dy);
        fill_truth_detection_supervisely(num_gt_boxes[index], boxes[index], num_boxes, d.y.vals[i], classes, flip, -dx/w, -dy/h, nw/w, nh/h, sized);
        free_image(orig);
    }
    free(random_indices);
    return d;
...
data load_data_supervisely_no_aug(int n, char **paths, float **boxes, int *num_gt_boxes, int m, int w, int h, int num_boxes, int classes)
{
    int *random_indices = get_random_indices(n, m);
    int i, index;
    // printf("%03d / %03d  >>", n, m);;
    ...
    for(i = 0; i < n; ++i){
        ...
        float new_ar = ((float)orig.w) / orig.h;
        float nw, nh;
...
void *load_thread(void *ptr)
{
    ...
    } else if (a.type == SUPERVISELY_DATA){
        *a.d = load_data_supervisely(a.n, a.paths, a.boxes, a.num_gt_boxes, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
    } else if (a.type == SUPERVISELY_DATA_NO_AUG){
        *a.d = load_data_supervisely_no_aug(a.n, a.paths, a.boxes, a.num_gt_boxes, a.m, a.w, a.h, a.num_boxes, a.classes);
    }
    free(ptr);
    return 0;
}
