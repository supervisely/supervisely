...
from supervisely.project.versioning.volume_snapshot import (
    DEFAULT_VOLUME_SNAPSHOT_SCHEMA_VERSION,
    get_volume_snapshot_handler,
    get_volume_snapshot_handler_from_blob,
)
...
class VolumeProject(VideoProject):
    ...
    def download_bin(
        ...
        schema_version: str = DEFAULT_VOLUME_SNAPSHOT_SCHEMA_VERSION,
        *args,
        **kwargs,
        ...
        handler = get_volume_snapshot_handler(schema_version)
        blob = handler.build_blob(
            api=api,
            project_id=project_id,
            dataset_ids=dataset_ids,
            download_volumes=download_volumes,
    ...
    def upload_bin(
        ...
        **kwargs,
        ...
        handler = get_volume_snapshot_handler_from_blob(raw_data)
        return handler.restore_blob(
            api=api,
            raw_data=raw_data,
            workspace_id=workspace_id,
            project_name=project_name,
            log_progress=log_progress,
            progress_cb=progress_cb,
            skip_missed_entities=skip_missed_entities,
        )
