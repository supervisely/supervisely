...
from supervisely.project.versioning.video_snapshot import (
    DEFAULT_VIDEO_SNAPSHOT_SCHEMA_VERSION,
    get_video_snapshot_handler,
)
...
class VideoProject(Project):
    ...
    def restore_snapshot(
        ...
        try:
            ...
            schema_version = (
                manifest.get("schema_version") or DEFAULT_VIDEO_SNAPSHOT_SCHEMA_VERSION
            )
            handler = get_video_snapshot_handler(schema_version)
            return handler.restore_payload(
                api=api,
                payload_dir=payload_dir,
                workspace_id=workspace_id,
                project_name=project_name,
