...
def resize_origin_and_bitmap(
    ...
    # Scaled origin requires rounding to match scaled bitmap size.
    scaled_row = round(origin.row * row_scale)
    scaled_col = round(origin.col * col_scale)

    scaled_origin = PointLocation(row=scaled_row, col=scaled_col)
