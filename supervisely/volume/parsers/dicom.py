from loaders.dicom import SliceStacker
...
photometricInterpretationRGB = set(['RGB', 'PALETTE COLOR', 'YBR_FULL', 'YBR_FULL_422', 'YBR_PARTIAL_422', 'YBR_PARTIAL_420', 'YBR_RCT'])
...
def load(entry_path):
    ...
    for file in os.listdir(entry_path):
        if file.lower().endswith('dcm'):
            ...
            si_uid = pydicom.read_file(full_path, stop_before_pixels=True)[SliceStacker.SI_UID_TAG].value
            suids.add(si_uid)
    ...
    for si_uid in suids:
        try:
            ...
            if hasattr(slice, 'PhotometricInterpretation') and slice.PhotometricInterpretation in photometricInterpretationRGB:
            ...
            sly.logger.warn('Series {} skipped due to error: {}'.format(si_uid, str(e)), exc_info=True, extra={
                'exc_str': str(e),
