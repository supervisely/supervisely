...
# @TODO: check extension from first bytes of file
...
def validate_ext(ext: str):
    ...
    if not is_valid_ext(ext):
        raise UnsupportedVolumeFormat(
            "Unsupported Volume extension: {}. Only the following extensions are supported: {}.".format(
                ext, ALLOWED_VOLUME_EXTENSIONS
            )
        )
