...
def validate_classes_exact_match(saved_classes: List[str], current_classes: List[str]):
    ...
    :param saved_classes: List of saved class names
    :type saved_classes: List[str]
    :param current_classes: List of current class names
    :type current_classes: List[str]
    :raises: ValueError if saved and current classes don't match
    :rtype: None
    """
...
def reorder_class_map(saved_classes: List[str], project_meta) -> ClassMap:
    ...
    :returns: New ClassMap with correct order
    :rtype: ClassMap
    """
...
def remove_classification_head(checkpoint_path: str) -> str:
    ...
    :returns: Path to checkpoint without classification head
    :rtype: str
    """
...
def resolve_checkpoint(
    ...
    :param checkpoint_mode: One of 'scratch', 'finetune', 'continue'
    :type checkpoint_mode: str
    :param selected_experiment_task_id: Task ID to load checkpoint from (required for finetune/continue)
    :type selected_experiment_task_id: Optional[int]
    :param class_map: Current ClassMap
    :type class_map: ClassMap
    ...
    :type team_id: int
    :param work_dir: Working directory for downloaded files
    :type work_dir: str
    :returns: Tuple containing checkpoint path, updated class map, and state (only for continue mode)
    :rtype: Tuple[Optional[str], ClassMap, Optional[dict]]
    """
...
def save_state_json(state: dict, checkpoint_path: str):
    ...
    :param state: State dict from :meth:`~supervisely.nn.live_training.live_training.LiveTraining.state`method
    :type state: dict
    :param checkpoint_path: Path to .pth checkpoint file
...
def load_state_json(checkpoint_path: str) -> dict:
    ...
    :param checkpoint_path: Path to .pth checkpoint file
    :type checkpoint_path: str
    :returns: State dict for :meth:`~supervisely.nn.live_training.live_training.LiveTraining.load_state`()
    :rtype: dict
    """
    state_path = checkpoint_path.replace('.pth', '_state.json')
