...
def prepare_segmentation_data(source_project_dir, output_project_dir, palette):
    if os.path.exists(output_project_dir):
        sly.logger.info(
            f"Preprocessed data already exists in {output_project_dir} directory"
        )
        return
    else:
        ...
        for dataset in datasets:
            ...
            mask_files = os.listdir(
                os.path.join(temp_project_seg_dir, dataset, ann_dir)
            )
...
def evaluate_segmentation_quality(
    ...
    gt_sly_dir, pred_sly_dir = (
        f"./sly_data/gt/{gt_project_id}",
        f"./sly_data/pred/{pred_project_id}",
    )
    download_projects(api, gt_project_id, pred_project_id, "sly_data")
    ...
    gt_prep_dir, pred_prep_dir = (
        f"./preprocessed_data/gt/{gt_project_id}",
        f"./preprocessed_data/pred/{pred_project_id}",
    )
    prepare_segmentation_data(gt_sly_dir, gt_prep_dir, palette)
    prepare_segmentation_data(pred_sly_dir, pred_prep_dir, palette)

    calculate_metrics(
        gt_dir=gt_prep_dir,
        pred_dir=pred_prep_dir,
        boundary_width=0.01,
        boundary_iou_d=0.02,
        num_workers=4,
