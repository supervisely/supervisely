...
class AugmentationsNew(Widget):
    def __init__(
        self,
        api: sly.Api,
        project_id: int,
        image_info: Optional[sly.ImageInfo] = None,
        task_type: Optional[Literal["detection", "segmentation"]] = None,
        remote_preview_path: Optional[str] = "/temp/preview_augs.jpg",
        ...
        self._project_meta = sly.ProjectMeta.from_json(api.project.get_meta(project_id))
    ...
    def preview_augs(self, image_info: sly.ImageInfo = None):
        ...
        image_ann = sly.Annotation.from_json(ann_json, self._project_meta)
        meta = self._project_meta
        if self._task_type == "detection":
            image_ann, meta = self.convert_ann_to_bboxes(image_ann, self._project_meta)

        _, res_img, res_ann = sly.imgaug_utils.apply(self._pipeline, meta, img, image_ann)
        local_image_path = os.path.join(self._temp_dir, "preview_augs.jpg")
        sly.image.write(local_image_path, res_img)
        if self._api.file.exists(self._team_id, self._remote_preview_path):
            self._api.file.remove(self._team_id, self._remote_preview_path)
        file_info = self._api.file.upload(
            self._team_id, local_image_path, self._remote_preview_path
        )

        self._grid_gallery.clean_up()
        self._grid_gallery.append(
            title="Original", image_url=self._image_info.full_storage_url, annotation=image_ann
        )
        self._grid_gallery.append(
            title="Augmented", image_url=file_info.full_storage_url, annotation=res_ann
        )
        self._grid_gallery.show()

    def get_random_image_info(self):
        api: sly.Api = self._api
    ...
    def convert_ann_to_bboxes(image_ann, project_meta):
        ...
        new_ann_json = {
            "size": {"height": image_ann.img_size[0], "width": image_ann.img_size[1]},
            "tags": [],
            "objects": [],
        }
    ...
    def load_augs_template(config: dict):
        pipeline = sly.imgaug_utils.build_pipeline(
...
class AugmentationsWithTabsNew(Widget):
    def __init__(
        ...
        self._augs1 = AugmentationsNew(
            api, project_id, image_info=self._image_info, task_type=self._task_type
        )
        self._augs2 = AugmentationsNew(
            api, project_id, image_info=self._image_info, task_type=self._task_type
        )
        ...
        def update_template():
            ...
            if self._api.file.exists(
                team_id=self._team_id, remote_path=custom_template_path.strip()
            ):
    ...
    def load_existing_pipeline(self, remote_path):
        custom_pipeline_path = os.path.join(
            self._temp_dir, sly.fs.get_file_name_with_ext(remote_path)
        )
        self._api.file.download(self._team_id, remote_path, custom_pipeline_path)
...
class Augmentations(Widget):
    ...
    def preview_augs(self, image_info: sly.ImageInfo = None):
        ...
        file_info = self._globals.api.file.upload(
            self._globals.team.id, local_image_path, self._remote_preview_path
        )
...
class AugmentationsWithTabs(Widget):
    def __init__(
        ...
        def update_template():
            custom_template_path = self._template_path_input.get_value()
            if self._globals.api.file.exists(
                team_id=self._globals.team.id, remote_path=custom_template_path.strip()
            ):
                self.load_existing_pipeline(custom_template_path)
                self._augs2._editor.show()
                self._augs2._button_preview.show()
    ...
    def load_existing_pipeline(self, remote_path):
        custom_pipeline_path = os.path.join(
            self._globals.data_dir, sly.fs.get_file_name_with_ext(remote_path)
        )
