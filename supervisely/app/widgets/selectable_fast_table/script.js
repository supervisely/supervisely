Vue.component('selectable-fast-table', {
  template: `
          ...
          >
            ...
            <div class="flex items-center gap-4">
              ...
              <div
                ...
                class="text-[.9rem] text-slate-500 flex items-center gap-0 whitespace-nowrap"
              >
                <button
                  aria-label="Go back"
  ...
  methods: {
    ...
    selectAllRowsIdx() {
      if (this.selectedIdxs.length === this.data.length && this.data.length > 0) {
        // Deselect all
        ...
        const max = this.settings?.maxSelectedRows;
        if (max) {
          const limitedRows = allRows.slice(-max);
          this.$emit('update:selected-rows', limitedRows);
        } else {
          this.$emit('update:selected-rows', allRows);
        }
      }
    },
    ...
    updateSelectedRows(row) {
      ...
      const exists = currentSelectedRows.some(r => r.idx === row.idx);

      if (exists) {
        newSelectedRows = currentSelectedRows.filter(r => r.idx !== row.idx);
      } else {
        newSelectedRows = [...currentSelectedRows, row];
        const max = this.settings?.maxSelectedRows;
        if (max) {
          newSelectedRows = newSelectedRows.slice(-max);
        }
      }
      this.selectedIdxs = newSelectedRows.map(r => r.idx);
      this.$emit('update:selected-rows', newSelectedRows);
    },
