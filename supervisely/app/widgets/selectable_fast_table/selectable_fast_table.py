...
class SelectableFastTable(Widget):
    """SelectableFastTable widget in Supervisely is an enhanced version of FastTable that allows for
    displaying and manipulating data with advanced row-level customization, improved selection features,
    and header content customization.
    ...
    .. code-block:: python
        ...
        from supervisely.app.widgets import SelectableFastTable

        # Example with row-level content injection
        ref_img_url = "https://example.com/image.png"
        data = [
            [("Class A", ref_img_url), 10],  # Tuple with image URL
            [("Class B", "custom_data"), 20],  # Tuple with custom text
            ["Class C", 15],  # Regular data
        ]
        columns = ["Class", "Items"]
        columns_options = [
            { "customCell": True, "type": "class"},
            { "maxValue": 21, "postfix": "pcs", "tooltip": "description text", "subtitle": "boxes" }
        ]

        # With header content and selection
        team_workspace_select = TeamWorkspaceSelect(show_label=True)
        selectable_table = SelectableFastTable(
            data=data,
            columns=columns,
            columns_options=columns_options,
            is_selectable=True,
            header_left_content=team_workspace_select,
        )
    """
        ...
        def widget_html(self):
            ...
            if hasattr(self.widget, "_widgets"):
                for i, widget in enumerate(self.widget._widgets):
                    ...
                    html = html.replace(
                        f"/{widget.widget_id}",
                        f"/' + JSON.parse(cellValue).widgets[{i}] + '",
                    )
            return html
    ...
    def __init__(
        ...
        script_path = "./sly/css/app/widgets/selectable_fast_table/script.js"
        JinjaWidgets().context["__widget_scripts__"][self.__class__.__name__] = script_path

        filter_changed_route_path = self.get_route_path(SelectableFastTable.Routes.UPDATE_DATA)
    ...
    def get_json_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "options": {
                ...
                "isSelectable": self._is_selectable,
    ...
    @property
    def max_selected_rows(self) -> int:
        return self._max_selected_rows

    @max_selected_rows.setter
    def max_selected_rows(self, value: int) -> None:
        self._max_selected_rows = value
        DataJson()[self.widget_id]["options"]["maxSelectedRows"] = value
    ...
    def set_sort(
        self,
        func: Callable[[pd.DataFrame, int, Optional[Literal["asc", "desc"]]], pd.DataFrame],
    ) -> None:
    ...
    def row_click(self, func: Callable[[ClickedRow], Any]) -> Callable[[], None]:
        ...
        :rtype: Callable[[], None]
        """
        row_clicked_route_path = self.get_route_path(SelectableFastTable.Routes.ROW_CLICKED)
        server = self._sly_app.get_server()
    ...
    def cell_click(self, func: Callable[[ClickedCell], Any]) -> Callable[[], None]:
        ...
        :rtype: Callable[[], None]
        """
        cell_clicked_route_path = self.get_route_path(SelectableFastTable.Routes.CELL_CLICKED)
        server = self._sly_app.get_server()
                ...
                for i, row in enumerate(data):
                    ...
                    for j, item in enumerate(row):
                        if isinstance(item, tuple) and len(item) == 2:
                            # Handle tuples like ("Class A", "image_url") or ("Class A", Widget)
                            display_value, special_data = item

                            if isinstance(special_data, str):
                                # If it's a string, treat it as special data (e.g., image URL)
                                self._row_special_data[i] = special_data
                                data[i][j] = display_value
                            elif issubclass(type(special_data), Widget):
                                # If it's a widget, use the old system for compatibility
                                prepare_html = (
                                    lambda w: w.to_html()
                                    .replace(
                                        f".{w.widget_id}",
                                        "[JSON.parse(cellValue).widget_id]",
                                    )
                                    .replace(
                                        f"/{w.widget_id}",
                                        "/' + JSON.parse(cellValue).widget_id + '",
                                ...
                                self._row_idx_to_preview[i] = prepare_html(special_data)
                                data[i][j] = display_value
                            else:
                                # Convert other types to string
                                self._row_special_data[i] = str(special_data)
                                data[i][j] = display_value
    ...
    def selection_changed(self, func):
        ...
        :rtype: Callable[[], None]
        """
        selection_changed_route_path = self.get_route_path(
            SelectableFastTable.Routes.SELECTION_CHANGED
        )
        server = self._sly_app.get_server()
    ...
    def select_row(self, idx: int):
        ...
        self._selected_rows = [
            {
                "idx": idx,
                "row": selected_row.get("items", selected_row.get("row", None)),
            }
        ]
        StateJson()[self.widget_id]["selectedRows"] = self._selected_rows
        StateJson().send_changes()
