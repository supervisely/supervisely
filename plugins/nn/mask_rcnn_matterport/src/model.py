...
class SuperViselyCallback(Callback):
    def __init__(self, epochs, batches, out_config, checkpoints_saver):
        ...
        self.cur_epoch = 0
        self.progress_counter = sly.Progress('Model training: ', epochs * batches)
        self.batches = batches
        self.out_config = out_config
        self.checkpoints_saver = checkpoints_saver
        self.best_val_loss = float('Inf')

    @staticmethod
    def _check_loss(loss, source_tag):
        if loss is None or np.any(np.isnan(loss)) or np.any(np.isinf(loss)):
            error_message = 'Invalid loss was encountered for {}: {}. Train process will be terminated. Please check your training setup.'.format(
                source_tag, str(loss))
    ...
    def on_batch_end(self, batch, logs=None):
        logs = logs or {}
        SuperViselyCallback._check_loss(logs.get('loss'), 'train')

        epoch_flt = self.cur_epoch + float(batch + 1) / float(self.batches)
        sly.report_metrics_training(epoch_flt, {'loss': float(logs.get('loss')[0][0])})
        self.progress_counter.iter_done_report()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        SuperViselyCallback._check_loss(loss, 'val')
        val_loss = float(loss[0][0])
        val_loss_dict = {'loss': val_loss}
        sly.report_metrics_validation(epoch + 1, val_loss_dict)
        model_is_best = val_loss < self.best_val_loss
        if model_is_best:
        ...
        weights_dir = os.path.join(out_dir, 'model_weights')
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    # Regularize the denominator slightly to avoid NaN losses with 0 active
    # classes.
    loss = tf.reduce_sum(loss) / (tf.reduce_sum(pred_active) + 1e-3)
    loss = K.reshape(loss, [1, 1])
    return loss
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    bad_masks_indx = np.where(np.sum(mask, axis=(0, 1)) == 0)[0]
    mask = np.delete(mask, bad_masks_indx, axis=2)
    class_ids = np.delete(class_ids, bad_masks_indx, axis=0)
    ...
    source_class_ids = dataset.active_classes_per_image(image_id)
...
def data_generator(dataset, config, shuffle=True, augment=False, random_rois=0,
    ...
    image_index = -1
    image_ids = np.arange(len(dataset.samples))
    error_count = 0
    ...
    while True:
        try:
            ...
            logging.exception("Error processing image {}".format(dataset.samples[image_id][0]))
            error_count += 1
            if error_count > 5:
                raise
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            model = KM.Model(inputs=[input_image, input_image_meta],
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        ...
        keras_model = keras_model if keras_model is not None else self.keras_model
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, out_config, sly_checkpoints_saver):
        """Train the model.
        ...
        # Callbacks
        callbacks = [
            SuperViselyCallback(epochs=epochs, batches=self.config.STEPS_PER_EPOCH,
                                out_config=out_config, checkpoints_saver=sly_checkpoints_saver),
        ]
        if val_dataset == None:
