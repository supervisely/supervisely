...
for current_app in apps_list_to_launch:
    app_default_modal_state = api.app.get_info_by_id(id=current_app.id).config.get('modalTemplateState', {})
    app_default_modal_state.update({'device': 'cpu'})  # update by custom params

    launched_tasks_list = api.task.start(
        agent_id=AGENT_ID,
        app_id=current_app.id,
        workspace_id=WORKSPACE_ID,
        params={'state': app_default_modal_state}
    )
